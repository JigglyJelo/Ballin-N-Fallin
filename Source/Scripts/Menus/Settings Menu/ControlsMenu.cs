...
public partial class ControlsMenu : VerticalMenu, ILeftRightSelections{
    public static string TargetProfile = ControlProfileManager.DEFAULT_PROFILE;
    ...
    public override void _Ready(){
        ...
        //Total selections = Number of Actions + 1 for Vibration (Profile selector is gone)
    ...
    protected override void MenuChoose(int choice){
        if(TargetProfile == ControlProfileManager.DEFAULT_PROFILE){
            SFX.Play("Error");
            return; //Default profile is read-only
        }

        if(choice == totalSelections){ // Last option is VIBRATION
        ...
        //Actions start at choice 1, so subtract 1 to get the array index
    ...
    public override void _Input(InputEvent @event){
        if(!isListening){
            if(Selection < totalSelections && @event is InputEventJoypadButton btnEvent && btnEvent.IsPressed()){
                ...
                if(btnEvent.ButtonIndex == JoyButton.X){ // Clear Binds
                    ControlProfileManager.ClearEventsFromProfile(TargetProfile, targetAction);
                    SFX.Play("Move"); 
                    UpdateTexts();
                    GetViewport().SetInputAsHandled();
                }else if(btnEvent.ButtonIndex == JoyButton.Y){ // Restore Default
